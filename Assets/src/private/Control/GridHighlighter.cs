...
public class GridHighlighter : MonoBehaviour
{
    [Header("Highlight Settings")]
    public Color lineColor = Color.black;
    public Material lineMat;      // If left empty, will create one at runtime
    public int patchSize = 25;    // Number of triangles to highlight
    public float lineOffset = 0.001f; // Offset to avoid z-fighting

    private int hoverTriangle = -1;
    private Mesh mesh;
    private Dictionary<int, List<int>> adjacentTriangles;
    ...
    void Start()
    {
        // Clone mesh to avoid modifying shared asset
        mesh = Instantiate(GetComponent<MeshFilter>().mesh);
        GetComponent<MeshFilter>().mesh = mesh;
        GetComponent<MeshCollider>().sharedMesh = mesh;
        ...
        Debug.Log($"Adjacency for tri 0: {string.Join(",", adjacentTriangles[0])}");
    ...
    void Update()
    {
        ...
        {
            hoverTriangle = hit.triangleIndex;
        }
        else
        {
            hoverTriangle = -1;
        ...
        // Only rebuild patch if triangle changed
        if (hoverTriangle != lastTriangle)
        {
            currentPatch = hoverTriangle >= 0 ? GetTrianglePatch(hoverTriangle, patchSize) : null;
            lastTriangle = hoverTriangle;
    ...
    void OnRenderObject()
    {
        ...
        foreach (int idx in currentPatch)
        {
    ...
    // BFS to find N nearest triangles
    private HashSet<int> GetTrianglePatch(int start, int max)
    {
        var visited = new HashSet<int> { start };
        ...
        {
            foreach (var neighbor in adjacentTriangles[queue.Dequeue()])
            {
    ...
    {
        adjacentTriangles = new Dictionary<int, List<int>>();
        int[] tris = mesh.triangles;
        int triCount = tris.Length / 3;

        for (int i = 0; i < triCount; i++)
            adjacentTriangles[i] = new List<int>();
        ...
        for (int i = 0; i < triCount; i++)
        {
            ...
            AddEdge(edgeMap, a, b, i);
            AddEdge(edgeMap, b, c, i);
            AddEdge(edgeMap, c, a, i);
        }

        // Link all triangles sharing each edge
        foreach (var kvp in edgeMap)
        {
            List<int> connected = kvp.Value;
            for (int i = 0; i < connected.Count; i++)
            {
                for (int j = i + 1; j < connected.Count; j++)
                {
                    ...
                    if (!adjacentTriangles[t1].Contains(t2))
                        adjacentTriangles[t1].Add(t2);

                    if (!adjacentTriangles[t2].Contains(t1))
                        adjacentTriangles[t2].Add(t1);
                }
            }
        }

        Debug.Log($"Adjacency built: {triCount} triangles, {edgeMap.Count} edges.");
    }



    private void AddEdge(Dictionary<(int, int), List<int>> map, int v1, int v2, int idx)
    {
        (int, int) key = (Mathf.Min(v1, v2), Mathf.Max(v1, v2));
